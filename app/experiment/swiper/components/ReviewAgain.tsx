...
const ReviewAgain = () => {
  ...
  const [reviewAgain, setReviewAgain] = useState<boolean[]>([]);
  ...
  useEffect(() => {
    ...
    if (storedWords) {
      ...
      setReviewAgain(new Array(words.length).fill(false)); // reviewAgainの初期化
    }
  }, []);
  ...
  //指定されたインデックスのwordのreviewAgainを変更して保存する
  ...
  const handleReviewAgain = () => {
    const wordsToReviewAgain = reviewWords.filter((_, i) => reviewAgain[i]);
    setReviewWords(wordsToReviewAgain);
    console.log(wordsToReviewAgain)
    setActiveIndex(0); // スライドを最初に戻す
  };
      ...
      >
        ...
        {reviewWords.map((card, i) => (
          <React.Fragment key={i}>
            <SwiperSlide>
              <div className="text-gray-400 pt-3 pl-3 ">
                語句
                <input
                  type="checkbox"
                  checked={reviewAgain[i]}
        ...
        <SwiperSlide>
          <div className="h-full flex flex-col items-center justify-center   bg-gradient-to-t from-yellow-300 to-orange-400 text-gray-100 p-8 rounded-lg shadow-xl">
            ...
            {reviewAgain.some((checked) => checked) && (
              <button
                onClick={handleReviewAgain}
                className="mt-4 px-4 py-2 border-2 font-semibold rounded-md shadow-sm hover:bg-gray-100 transition duration-300"
              >
                もう一度復習する
              </button>
            )}
